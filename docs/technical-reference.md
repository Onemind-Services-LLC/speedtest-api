...
To investigate a reported `X-Request-ID` on a VM, read its JSON entry and then filter by the shared `test_id` (one test) or `client_id` (the anonymous tab session):

```sh
sudo journalctl -u speedtest-api --since '1 hour ago' -o cat | jq -c 'select(.request_id == "REQUEST_ID")'
sudo journalctl -u speedtest-api --since '1 hour ago' -o cat | jq -c 'select(.test_id == "TEST_ID")'
```

The same fields are available in Kubernetes container logs. Session IDs do not identify a person across browsers or devices and can be supplied by any client. Keep log retention appropriate for operational troubleshooting.
